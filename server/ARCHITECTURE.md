...
approval 的最终 `approved/rejected` 状态以 agent ack 为准。用户点击决定后，server 先记录 `decision_command_id` 和 `decision_queued_at`，表示决定已排队等待 agent 接收；只有 agent ack 接受决定后，`approval_requests.status` 才更新为最终状态。
