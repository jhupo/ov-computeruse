...
server 是多用户控制平面，负责 agent 绑定、设备管理、Codex 索引存储、命令下发、运行事件广播、审计和余额/key 校验入口。server 不执行 Codex，不保存本地项目文件系统，也不把私钥或运行密钥打进镜像。
...
- `users` / `user_keys`：用户、余额和允许使用的 Codex key fingerprint。
