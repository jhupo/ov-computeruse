...
func TestRuntimeTimelineSessionQueryAcceptsSessionOrThreadID(t *testing.T) {
	...
	for _, want := range []string{"session_id=$2 OR thread_id=$2", "history AS", "FROM history_items hi", "LEFT JOIN LATERAL", "LIMIT 1", "live.item_id=COALESCE(NULLIF(hi.source_event_id, ''), hi.id)", "WHEN hi.kind='message' AND COALESCE(hi.role, '')='user' THEN 'user.message'", "ORDER BY event_at ASC"} {
		if !strings.Contains(query, want) {
