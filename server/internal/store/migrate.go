...
func (s *Store) migrate(ctx context.Context) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS history_item_uploads (agent_id TEXT NOT NULL REFERENCES agents(id), session_id TEXT NOT NULL, upload_id TEXT NOT NULL, cursor TEXT, batch_count INTEGER NOT NULL, final_seen BOOLEAN NOT NULL DEFAULT false, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY(agent_id, session_id, upload_id))`,
		`CREATE TABLE IF NOT EXISTS history_item_upload_batches (agent_id TEXT NOT NULL REFERENCES agents(id), session_id TEXT NOT NULL, upload_id TEXT NOT NULL, batch_index INTEGER NOT NULL, received_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY(agent_id, session_id, upload_id, batch_index))`,
		`CREATE TABLE IF NOT EXISTS history_item_staging (agent_id TEXT NOT NULL REFERENCES agents(id), session_id TEXT NOT NULL, upload_id TEXT NOT NULL, batch_index INTEGER NOT NULL, item_index INTEGER NOT NULL, role TEXT, kind TEXT NOT NULL, text TEXT, payload JSONB, source TEXT, source_event_id TEXT, item_at TIMESTAMPTZ, received_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY(agent_id, session_id, upload_id, batch_index, item_index, kind))`,
		...
		`ALTER TABLE runtime_sessions ADD COLUMN IF NOT EXISTS profile TEXT`,
		`ALTER TABLE runtime_sessions ADD COLUMN IF NOT EXISTS approval_policy TEXT`,
		`ALTER TABLE runtime_sessions ADD COLUMN IF NOT EXISTS sandbox_mode TEXT`,
		`ALTER TABLE runtime_sessions ADD COLUMN IF NOT EXISTS reasoning_effort TEXT`,
		`ALTER TABLE runtime_sessions ADD COLUMN IF NOT EXISTS last_turn_id TEXT`,
		`ALTER TABLE runtime_sessions ADD COLUMN IF NOT EXISTS last_item_index INTEGER NOT NULL DEFAULT 0`,
		...
		`CREATE TABLE IF NOT EXISTS run_event_gaps (id TEXT PRIMARY KEY, agent_id TEXT NOT NULL REFERENCES agents(id), run_id TEXT NOT NULL, expected_seq BIGINT NOT NULL, observed_seq BIGINT NOT NULL, kind TEXT NOT NULL, status TEXT NOT NULL, existing_event_id TEXT, observed_event_id TEXT, existing_kind TEXT, observed_kind TEXT, existing_payload_sha256 TEXT, observed_payload_sha256 TEXT, details JSONB, detected_at TIMESTAMPTZ NOT NULL DEFAULT now(), resolved_at TIMESTAMPTZ)`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS existing_event_id TEXT`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS observed_event_id TEXT`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS existing_kind TEXT`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS observed_kind TEXT`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS existing_payload_sha256 TEXT`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS observed_payload_sha256 TEXT`,
		`ALTER TABLE run_event_gaps ADD COLUMN IF NOT EXISTS details JSONB`,
		...
		`CREATE TABLE IF NOT EXISTS runtime_timeline (id TEXT PRIMARY KEY, agent_id TEXT NOT NULL REFERENCES agents(id), run_id TEXT NOT NULL, session_id TEXT, project_id TEXT, seq BIGINT NOT NULL, runtime TEXT NOT NULL, thread_id TEXT, turn_id TEXT, item_id TEXT, item_type TEXT, phase TEXT, kind TEXT NOT NULL, role TEXT, text TEXT, status TEXT, payload JSONB, event_at TIMESTAMPTZ NOT NULL, received_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(agent_id, run_id, seq, kind, item_id, phase))`,
	...
	indexes := []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_history_item_upload_batches ON history_item_upload_batches(agent_id, session_id, upload_id)`,
		`CREATE INDEX IF NOT EXISTS idx_history_item_staging_upload ON history_item_staging(agent_id, session_id, upload_id, batch_index)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_runtime_timeline_run ON runtime_timeline(agent_id, run_id, seq)`,
		`CREATE INDEX IF NOT EXISTS idx_runtime_timeline_thread ON runtime_timeline(agent_id, thread_id, event_at) WHERE thread_id IS NOT NULL AND thread_id <> ''`,
		`CREATE INDEX IF NOT EXISTS idx_runtime_timeline_session ON runtime_timeline(agent_id, session_id, event_at) WHERE session_id IS NOT NULL AND session_id <> ''`,
