...
`state.db` 保存 Codex roots、项目索引、会话索引、runtime session 映射、run 本地状态投影、run event 本地事实源/outbox、history chunk 发送/确认状态和 server sync cursors。agent 启动时会把未终止的本地 run 收敛为 interrupted 事件并进入 outbox，避免崩溃后永久停留在 running。它不保存 Codex 原始 auth/config 副本，不保存 OpenAI API key 明文。
