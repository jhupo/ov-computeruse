...
`state.db` stores discovered Codex roots, project/session indexes, runtime session mappings, local run status/message/tool-call projections, the local run event outbox, history chunk upload state, and sync cursors. On startup, unfinished local runs are reconciled as interrupted events and retried through the outbox. It does not store raw API keys or copied Codex auth files.
...
The build injects the server URL and deployment bind token. User keys and per-device `agent_secret` are never included in the package.
