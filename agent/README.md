...
`state.db` stores discovered Codex roots, project/session indexes, runtime session mappings, local run status projections, the local run event outbox, history chunk upload state, and sync cursors. On startup, unfinished local runs are reconciled as interrupted events and retried through the outbox. It does not store raw API keys or copied Codex auth files.
